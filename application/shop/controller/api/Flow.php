...
class Flow extends ApiController
{
    ...
    public function addOrder()
    {
        ...
        $address_id = input('address_id', 0, 'intval');
        if ($address_id < 0) return $this->error('请设置收货地址后再操作');
        $UserAddressModel = new UserAddressModel();
        $addressList = $UserAddressModel->getRows();
        $address = $addressList[$address_id];
        ...
        foreach ($cartList['goodsList'] as $key => $grow) {
            ...
            // 判断是商品能否购买或修改
            $res = $this->Model->checkGoodsOrder($goods, $grow['goods_number'], $grow['sku_val'], $grow['prom_type'], $grow['prom_id'], $promInfo);
            if ($res !== true) return $this->error($res['msg']);

            $price = $GoodsModel->evalPrice($goods, $grow['goods_number'], $grow['sku_val'], $grow['prom_type'], $grow['prom_id'], $promInfo);//计算需显示的商品价格
