...
class WeiXinUsersModel extends BaseModel
{
    ...
    public function login($access_token){
        ...
        $xcx_openid = session('xcx_openid');
        if (empty($xcx_openid) == false){
            $xcxWhere['wx_openid'] = $xcx_openid;
            $xcxWhere['is_xcx'] = 1;
            $xcx_info = $this->where($xcxWhere)->find();
            if (empty($xcx_info) == true){//不存在插入
                if (empty($wx_info) == false){
                    $xcxInArr['user_id'] = $wx_info['user_id'];
                }
                $xcxInArr['unionid'] = $wx_info['unionid'];
                $xcxInArr['is_xcx'] = 1;
                $xcxInArr['wx_openid'] = $xcx_openid;
                $xcxInArr['add_time'] = $xcxInArr['update_time'] = time();
                $this->save($xcxInArr);
            }else{
                $user_id = $xcx_info['user_id'];
            }
        }
        ...
        //查询是否已在小程序端触发生成会员,通过unionid查询
        if ($user_id < 1 && empty($wx_arr['unionid']) == false){
            $xcxWhere['unionid'] = $wx_arr['unionid'];
            $xcxWhere['is_xcx'] = 1;
            $user_id = $this->where($xcxWhere)->value('user_id');
        }

		$inarr['user_id'] = $user_id;
		$inarr['wx_openid'] = $access_token['openid'];
        $inarr['unionid'] = $wx_arr['unionid'];
