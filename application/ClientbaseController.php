...
class ClientbaseController extends BaseController{
    ...
    public function initialize()
    {
        ...
        $xcx_openid = input('xcx_openid','','trim');
		if (empty($xcx_openid) == false){
		    $this->checkXcxOpenid($xcx_openid);
        }
        //$this->is_wx = 1;//本地测试使用
    ...
    protected function checkLoginUser(){
        ...
        if (empty($this->userInfo) == true) {
            ...
            if ($this->is_wx == 1 && empty(settings('weixin_appid')) == false) {//微信网页访问执行
                ...
                if (empty($wxInfo) == true && settings('weixin_auto_login') != 2) {
    ...
    //-- 验证网页传递xcxopenid是否合法
    /*------------------------------------------------------ */
    protected function checkXcxOpenid($xcx_openid){
        $timestamp = input('timestamp',0,'intval');
        if ($timestamp < time() && time() - $timestamp > 10){
            return false;
        }
        $apiKey = \think\facade\Cache::get('xcx_apikey_'.$xcx_openid);
        $mySgin = md5($xcx_openid.'_'.$timestamp.'_'.$apiKey);
        $sgin = input('sgin','','trim');
        if ($mySgin != $sgin){
            return false;
        }
        session('xcx_openid',$xcx_openid);
    }
    /*------------------------------------------------------ */
    //-- 退出
    /*------------------------------------------------------ */
