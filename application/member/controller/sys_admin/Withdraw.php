...
//            }
    ...
    public function afterEdit($data)
    {
        ...
        if ($data['status'] == 9 && $data['operating'] == 'weixin') {

            $code = str_replace('/', '\\', "/payment/" . 'weixin' . "/" . 'weixin');
            $payment = new $code();

            $wxuser = Db::name('weixin_users')->where('user_id', $info['user_id'])->find();

            $res = $payment->sendMoney($info['amount'], $wxuser['wx_openid'], '提现', $wxuser['wx_nickname']);
            if ($res !== 'SUCCESS') {
                Db::rollback();//回滚
                return $this->error('未知错误，提现失败.');
            }
        }
        Db::commit();// 提交事务
